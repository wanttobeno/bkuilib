...
public:
    ...
    void SetSkin(HBITMAP hbmpSkin, BOOL bSingle = FALSE, BOOL bHasDisable = FALSE, COLORREF crText = CLR_INVALID, BOOL bAutoResize = TRUE)
    {
        ...
        m_imgSkin.SetTransparentMode(CBkImage::ModeMaskColor);
    ...
    void DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
    {
        ...
        if (m_bHorzExtend)
        {
            CBkSkinBase::HorzExtendDraw(dc, m_imgSkin, rcBtn, m_lHorzExSkinLeft, nSubImage);
        }
        else
        {
...
public:
    ...
    void GradientFillRectV(HDC hdc, CRect rcFill, FRG_PARAM params[], int nCount)
    {
        ...
        for (i = 1; i < nCount && vert[0].y <= rcFill.bottom; i ++)
        {
            ...
            CBkSkinBase::GradientFill2(hdc, vert, 2, &gRect, 1, GRADIENT_FILL_RECT_V);
        }
    }
    ...
    void OnPaint(CDCHandle /*dc*/)
    {
        ...
        hftOld = dc.SelectFont(BkFontPool::GetFont(BKF_DEFAULTFONT));
    ...
    void CreateSortBitmaps()
    {
        ...
        if (hShell != NULL)
        {
            ...
            for(int i = m_iSortUp; i <= m_iSortDown; i++)
            {
                ...
                m_bmSort[i].SetTransparentMode(CBkImage::ModeMaskColor);
...
public:
    ...
    void OnPaint(CDCHandle /*dc*/)
    {
        ...
        HFONT hftOld = dc.SelectFont(BkFontPool::GetFont(FALSE, TRUE, FALSE));
...
//         }
    ...
    void OnTimer(UINT_PTR nIDEvent,void* pNull = NULL)
    {
        if (1 != nIDEvent)
            return;
